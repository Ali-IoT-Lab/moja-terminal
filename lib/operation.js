...
mojaOperation.prototype.download=function (file) {
  var req = http.get(`http://47.97.210.118/xxxxxxxxxxx?uploadPath=${Base64.encode(}`,function (res) {
    fs.createReadStream(path.resolve(file)).pipe(request.put('http://mysite.com/obj.json'))
});
