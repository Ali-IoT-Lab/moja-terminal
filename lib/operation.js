...
mojaOperation.prototype.init=function () {
  ...
  setKey.stdout.on("data", (trunk) => {
    uout += trunk;
    if(trunk.length > 5 ){
      if(trunk.indexOf('npm http') == -1 && trunk.indexOf('http fetch') == -1 &&  trunk.indexOf('npm http fetch') == -1){
        if(trunk.indexOf('GET') > -1 && trunk.indexOf('cnpmjs') > -1)
        {
          var tmp = trunk.split('GET')[1];
          console.log()
