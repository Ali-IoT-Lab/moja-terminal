...
var debug = require('debug')('moja:paths');
var p     = require('path');
...
function getDefaultPM2Home() {
  ...
  if (process.env.PM2_HOME)
    ...
    PM2_ROOT_PATH = p.resolve(process.env.HOME, '.moja');
  else if (process.env.HOME || process.env.HOMEPATH)
    PM2_ROOT_PATH = p.resolve(process.env.HOMEDRIVE, process.env.HOME || process.env.HOMEPATH, '.moja');
  else {
    console.error('[MOJA][Initialization] Environment variable HOME (Linux) or HOMEPATH (Windows) are not set!');
    console.error('[MOJA][Initialization] Defaulting to /etc/.moja');
    PM2_ROOT_PATH = p.resolve('/etc', '.moja');
  }
  debug('moja home resolved to %s', PM2_ROOT_PATH, process.env.HOME);
...
module.exports = function(PM2_HOME) {
  ...
  var pm2_file_stucture = {
    ...
    PM2_LOG_FILE_PATH        : p.resolve(PM2_HOME, 'moja.log'),
    PM2_PID_FILE_PATH        : p.resolve(PM2_HOME, 'moja.pid'),
    ...
    PM2_IO_ACCESS_TOKEN      : p.resolve(PM2_HOME, 'moja-io-token'),
    DUMP_FILE_PATH           : p.resolve(PM2_HOME, 'dump.moja'),
    DUMP_BACKUP_FILE_PATH    : p.resolve(PM2_HOME, 'dump.moja.bak'),
