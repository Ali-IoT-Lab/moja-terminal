...
const monitorShell = require('../common/monitor.js');
...
monitor.prototype.totalUsage = function (callback) {
  ...
  exec(monitorShell.usage, function (error, stdout, stderr) {
    if(error){
...
monitor.prototype.cpuCode = function () {
  return {logicCpu:logicalCpuCount,physicalCpu:physicalCpuCount,time:nanoSecond()};
};

monitor.prototype.sendData = function (data,callback) {
	var zipData = JSON.stringify(data);

	zlib.gzip(zipData, function (err, buffer) {
    var options = {
...
var msmonit = module.exports = new monitor;
